...
from kci.GaussianKernel import GaussianKernel
from kci.Kernel import Kernel
from kci.LinearKernel import LinearKernel
from kci.PolynomialKernel import PolynomialKernel
...
def unconditional_independence(X: np.ndarray,
                               ...
                               polyd=2,
                               kwidthx=None,
                               kwidthy=None) -> AnalysisResult:
    X, Y = shape_into_2d(X, Y)
    p_value, test_statistic = KCI_UInd(kernelX=kernelX,
                                       kernelY=kernelY,
                                       null_ss=null_ss,
                                       approx=approx,
                                       est_width=est_width,
                                       polyd=polyd,
                                       kwidthx=kwidthx,
                                       kwidthy=kwidthy).compute_pvalue(X, Y)
...
def conditional_independence(X: np.ndarray,
                             ...
                             kwidthx=None,
                             kwidthy=None,
                             kwidthz=None) -> AnalysisResult:
    X, Y, condition_on = shape_into_2d(X, Y, condition_on)
    p_value, test_statistic = KCI_CInd(kernelX=kernelX,
                                       kernelY=kernelY,
                                       kernelZ=kernelZ,
                                       nullss=null_ss,
                                       approx=approx,
                                       ...
                                       kwidthx=kwidthx,
                                       kwidthy=kwidthy,
                                       kwidthz=kwidthz).compute_pvalue(X, Y, condition_on)
    return AnalysisResult(p_value=p_value, test_statistic=test_statistic)
